function [ drought droughtclasses droughtA droughtMA droughtMAM droughtMAS droughtMAE droughtADD droughtADS droughtADI droughtDSI] = STAND_LS_NCDA( indicator,mask,n,nondata,tini,tfin )
...
jj=1;
while opc;
   datay=droughtA(ii:ii+(n-1),1);
   droughtMA(jj,:)=datay(:);
   
   datay=droughtA(ii:ii+(n-1),2);
   droughtMAM(jj,:)=datay(:);
   
   datay=droughtA(ii:ii+(n-1),3);
   droughtMAS(jj,:)=datay(:);
   
   datay=droughtA(ii:ii+(n-1),4);
   droughtMAE(jj,:)=datay(:);
   ii=ii+n;
   jj=jj+1;
   if ii>=length(droughtA)
       opc=0;
   end
end
...
for ii=1:length(data(:,1))
    for jj=1:length(data(1,:))
        ...
        if mask(ii,jj)==1
            ...
            indicatorXY(1,:)=indicator(ii,jj,:);  %Analysis in TIME
            ...
            droughtDSI{ii,jj,1}=vectorDS(:,1);  %duration
            droughtDSI{ii,jj,2}=vectorDS(:,2);  %severity(magnitude)
            
            %droughtDSI{ii,jj,2}=vectorDS(:,2)/mean(vectorDS(:,2));  %severity(magnitude)
            
            if sum(vectorDS(:,1))==0
                aux=zeros(size(vectorDS(:,1)));
                droughtDSI{ii,jj,3}=aux(:,1);  %intensity
            else
                droughtDSI{ii,jj,3}=vectorDS(:,2)./vectorDS(:,1);  %intensity
            end
            
            
        else
            droughtDSI{ii,jj,1}=0;
            droughtDSI{ii,jj,2}=0;
            droughtDSI{ii,jj,3}=0;
        end

        droughtADD(ii,jj)=mean(droughtDSI{ii,jj,1});
        droughtADS(ii,jj)=mean(droughtDSI{ii,jj,2});
        droughtADI(ii,jj)=mean(droughtDSI{ii,jj,3});
